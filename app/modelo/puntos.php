...
class Puntos{
    ...
    public function getPuntos($id_torneo) {
        try {
            $query= $this->pdo->prepare("SELECT  e.nombre as equipo,t.nombre as torneo, sum(p.puntos) as puntos from puntos p 
                        ...
                        left join torneos t on (t.id=pt.id_torneo) where pt.id_torneo=? group by (e.nombre) order by puntos desc;
                        ");
            $query->execute([$id_torneo]);
